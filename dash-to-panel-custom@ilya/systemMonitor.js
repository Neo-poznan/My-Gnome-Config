...
var SystemMonitor = class SystemMonitor {
    ...
    createWidget(panel) {
        ...
        this._container = new St.BoxLayout({
            ...
            can_focus: true,
            track_hover: true,
            width: 400  // Фиксированная ширина для предотвращения смещений
        });
    ...
    _updatePosition() {
        ...
        // Проверяем видимость
        if (this._settings.get_boolean('hide-sysmon')) {
            return;
        }
        
        // Получаем позицию и добавляем
        const position = this._settings.get_string('sysmon-position');
    ...
    _updateFontSize() {
        ...
        const fontSize = this._settings.get_int('sysmon-font-size');
